	...
	# so all locations will be consistent
		...
		if ( $debug_arg && $debug_arg eq "mini-test" ) {
			...
			if ( $host eq "liberace" ) {
				...
				my $zy1 = DefaultDo("2010", "abs") ;
				unless ( $zy1 eq "ok" ) { Log(" $zy1" ); }
				...
				Log(" Starting normal default build sequence for abs 6150...");
				my $z7 = DefaultDo("6150", "324x") ;
				unless ( $z7 eq "ok" ) { Log(" $z7" ); }
				

				Log(" Starting normal default build sequence for abs 4920 ...\n");
				my $zx8 = DefaultDo("4920", "abs") ;
				unless ( $zx8 eq "ok" ) { Log(" $zx8" ); }
				...
				#Log(" Starting normal default build sequence for 330k 7200fab...");
				#my $a1 = DefaultDo("7200fab", "330k") ;
				#unless ( $a1 eq "ok" ) { Log(" $a1" ); }
				...
				#Log(" Starting normal default build sequence for abs 6150...");
				#my $y7 = DefaultDo("6150", "abs") ;
				#unless ( $y7 eq "ok" ) { Log(" $y7" ); }
				
				if (CheckServer("liberace") eq "bad" ){
					...
					Log(" Starting normal default build sequence for bs 7300...");
					my $p9 = DefaultDo("7300", "abs") ;
					unless ( $p9 eq "ok" ) { Log(" $p9" ); }
