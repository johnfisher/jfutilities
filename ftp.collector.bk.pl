...
my @servers = ("liberace" , "tiger" ,  "diz" , "pt",   "bobcat") ;
#my @servers = (   "pt" ) ;
...
my $tracefile_spec = "/tmp/ftp.collector.bk.log" ;
my $tracefile = "> $tracefile_spec" ;
...
<<<<<<< ftp.collector.bk.pl
	    if ( $day eq "Fri" ) {
	chdir $older_storefile_path or Expire("ProcessWeeklyMonthlyTarballs:  couldn't cd to $older_storefile_path  ");
=======
	    if ( $day eq "Wed" ) {
	#chdir $older_storefile_path or Expire("ProcessWeeklyMonthlyTarballs:  couldn't cd to $older_storefile_path  ");
>>>>>>> 1.21
...
TT("all my vars: last1weekdir " .   $last1weekdir .  " older_storefile_path " . $older_storefile_path . " last2weekdir " . $last2weekdir . " lastweekdir " . $lastweekdir . " thisweekdir " . $thisweekdir . " storefile_path ". $storefile_path . "  month " .$month);
	#copy($last1weekdir . '/*' , $last2weekdir);
TT(" error1: $!");
	    system " cp ". $last1weekdir . '/* ' . $last2weekdir ;
	#copy last to last+1
	#copy($lastweekdir . '/* ', $last1weekdir ) ;
TT(" error2: $!");

	    system " cp ". $lastweekdir . '/* ' . $last1weekdir ;
	#copy week_this to week_last
TT(" copy " . $thisweekdir  . '/* ' . $lastweekdir);
	#copy( $thisweekdir . '/*', $lastweekdir);
TT(" error3: $!");

	   system " cp ". $thisweekdir . '/* ' . $lastweekdir ;
	# copy day to week_this
TT(" copy". $storefile_path  . "*.tgz " . $thisweekdir);
	#copy($storefile_path  . "*.tgz", $thisweekdir);
TT(" error4: $!");
...
TT(" copy ". $storefile_path  . "*.tgz " . $month);
	#copy($storefile_path  . "*.tgz", $month);
TT(" error5: $!");
